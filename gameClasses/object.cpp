...
//create game object
GameObject::GameObject(GameController *game, sf::RenderWindow& window, std::vector<GameObject*> &gameObjects, std::string classType, std::vector<std::vector<int>> &currentObjects, sf::Texture *spriteSheet, int placeX_, int placeY_, bool movable, bool solid, bool animated, bool selfMoving, bool moveWithKeys, bool wander, int movingSpaceX, int movingSpaceY, int movingDirection, int currentSprite_, int animatedSpeed_, int sizeX, int sizeY, int spriteSheetSizeX, int spriteSheetSizeY, std::vector<int> spriteOrder_, std::vector<int> path, std::vector<int> extraInfo, bool cameraFocus) :
  window(window),
  ...
  game(game)
{
  ...
  movingObject = new MovingObject(game, sprite, sizeX, sizeY, spriteSheetSizeX, currentSprite, addedX, addedY, placeX, placeY, spriteOrder, animatedSpeed, spriteSheet->getSize().x / sizeX, cameraFocus);
